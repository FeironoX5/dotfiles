#!/usr/bin/env bash
# Todoist MCP server wrapper — reads token from ~/.config/secrets/todoist_token
# The token itself is NEVER stored in dotfiles.
# Run scripts/setup-mcp-secrets.sh to initialise your local secrets.
set -euo pipefail

TOKEN_FILE="$HOME/.config/secrets/todoist_token"
...
TODOIST_API_TOKEN="$(< "$TOKEN_FILE")"

exec npx -y @greirson/mcp-todoist